...
要求：Apple Silicon Mac、macOS 13+、QQ 已登录、Node.js 22+、pnpm 12.5.1。
...
3. 打开“系统设置 → 隐私与安全性 → 辅助功能”，给运行程序的终端或 `jev-jarvis.app` 授权。
...
5. 如果显示 `404`，说明网关没有注册 `jev-latest`，通常是 Decision Infra 进程没有拿到 `TYPESAFE_API_KEY`。
6. 如果显示 `503`，说明网关或它背后的 Provider 不可用。应用不会改用其他模型。
7. 切到浏览器后面板应隐藏；回到 QQ 后重新读取当前可见消息。
8. 同一屏里放两条连续的对方消息（其中一条较长）和一条自己的回复：应依次出现两张完整分析卡、一条紧凑的“我 · 上下文”条；状态只计数两条对方消息。滑到别处再滑回来，已经分析过的消息应从进程内缓存回显，不重复请求；新看到的对方历史消息会分析。现在优先读取每条消息行内名称为“我”或对方显示名的发送者容器；如果该节点没暴露，才用 AX 头像或明显的左右位置回退。
...
| 发送者 | 取消息行的发送者容器名称；引用内容里的名字不作为发送者 |
...
打开悬浮窗里的“模型设置”，界面只显示“判断 · Decision Infra”。也可以编辑 `~/.config/jev-jarvis/env`：
...
当前 HUD 不创建生成器、不使用 OpenAI/Anthropic 配置、不启动候选生成或候选排序线程，也不显示“复制”“填入”按钮。`DECISION_INFRA_MODEL` 是精确路由；不存在时直接报错，不 fallback。
