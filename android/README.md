...
在自己的 QQ 账号中打开一个聊天，放入两条对方文字、一条自己的文字，以及可识别的图片或文件。预期对方文字各有分析卡，自己的文字只显示“我 · 上下文”，媒体显示固定占位且不触发模型。分析卡默认展示判断内容，依次为原话、意图与情绪、回复风险、是否值得回复、意图候选、行为与需要、信号和带分数的下一步；在面板内滚动可查看下方内容，右上角“−”用于折叠整个面板。按住面板标题可拖动位置，拖动底部“调整大小”可改变宽高；位置和大小会保存在手机上。离开 QQ，面板应隐藏；返回聊天后可再次出现。用 QQ 的引用功能检查引用是否与新写正文分开。

如果没有出现面板，先按 [Android 无面板排查步骤](../docs/FAQ.md#android-qq-聊天页没有悬浮面板怎么办) 检查辅助功能和后台省电设置。返回 Jev · QQ 设置页，可查看底部“最近 QQ 识别状态”；它可区分尚未进入聊天页、会话标题未识别和当前没有可分析文字。
